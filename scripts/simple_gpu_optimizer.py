...
def get_optimal_batch_size(model_size='yolov5s', img_size=640, safety_factor=0.8):
    """
    根据GPU显存计算最优批次大小
    ...
    # 选择显存最大的GPU
    best_gpu = max(gpu_info['devices'], key=lambda x: x['total_memory'])
...
def optimize_training_config(model_size='yolov5s', img_size=640):
    """
    生成优化的训练配置
    ...
    Returns:
        优化的训练配置字典
    """
    batch_size = get_optimal_batch_size(model_size, img_size)
    ...
    config = {
        ...
        'workers': workers,
        'img_size': img_size,
        'device': '0' if gpu_info['available'] else 'cpu',
        'amp': gpu_info['available'],  # 只在GPU上启用混合精度
...
def monitor_gpu_memory(device_id=0):
    """
    监控GPU内存使用情况
    ...
    if device_id >= len(gpu_info['devices']):
    ...
    memory_info = {
        'device_id': device_id,
        'device_name': gpu_info['devices'][device_id]['name'],
        ...
        'total': gpu_info['devices'][device_id]['total_memory'],
        'total_gb': gpu_info['devices'][device_id]['total_memory_gb'],
    }
...
def print_optimization_report(model_size='yolov5s', img_size=640):
    """
    打印优化报告
    ...
    # 优化配置
    config = optimize_training_config(model_size, img_size)
    logger.info("=== 推荐训练配置 ===")
...
def main():
    ...
    for model in models:
        ...
        print_optimization_report(model)
